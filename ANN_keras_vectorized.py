...
def main(embedding_dim,n_neuron,lrate,remove_stop_words,n_rec_layer):
    ...
    if n_rec_layer == 1:
        ...
        model.add(layers.Dense(n_neuron, activation='softmax'))
        ...
        #input_length is how many words/units you want to embed 
        model.add(layers.Embedding(vocab_size, embedding_dim, input_length=max_length))
        model.add(layers.Dense(n_neuron, activation='relu')) #output_dim is the number of neurons in the recurrent layer #256 neurons
        model.add(layers.Dropout(0.5))
        model.add(layers.Dense(n_neuron, activation='relu'))
        model.add(layers.Dropout(0.5))
        model.add(layers.Dense(n_neuron, activation='relu'))
