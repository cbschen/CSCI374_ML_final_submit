...
def main(embedding_dim,n_neuron,lrate,remove_stop_words,n_rec_layer):
    ...
    if n_rec_layer == 1:
        ...
        model.add(layers.CuDNNLSTM(n_neuron)) #output_dim is the number of neurons in the recurrent layer #256 neurons
        ...
        #input_length is how many words/units you want to embed 
        model.add(layers.Embedding(vocab_size, embedding_dim, input_length=max_length))
        model.add(layers.CuDNNLSTM(n_neuron, return_sequences=True)) #output_dim is the number of neurons in the recurrent layer #256 neurons
        model.add(layers.Dropout(0.5))
        model.add(layers.CuDNNLSTM(n_neuron))
        model.add(layers.Dropout(0.5))
        model.add(layers.Dense(num_label, activation='softmax'))
        model.compile(optimizer=tf.keras.optimizers.Adam(lrate),loss='categorical_crossentropy',metrics=['accuracy'])
        ...
        model.add(layers.CuDNNLSTM(n_neuron, return_sequences=True)) #output_dim is the number of neurons in the recurrent layer #256 neurons
        model.add(layers.Dropout(0.5))
        model.add(layers.CuDNNLSTM(n_neuron, return_sequences=True))
        model.add(layers.Dropout(0.5))
        model.add(layers.CuDNNLSTM(n_neuron))
        model.add(layers.Dropout(0.5))
        model.add(layers.Dense(num_label, activation='softmax'))
